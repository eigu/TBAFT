namespace Runemark.SCEMA
{
    ...
    public class SCEMAWindow : EditorWindow
    {
        ...
        void DrawList(Rect rect, List<Location> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (!location.name.ToLower().Contains(searchField.ToLower())) continue;
        ...
        void DrawElement(Location location)
        {
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(5);
            BuildSettingsControl(location.Scene);
            ...
            LoadSceneControl(location.Scene);
        ...
        void Refresh()
        {
            ...
            string[] locationGUIDs = AssetDatabase.FindAssets("t:Location");
            settings.locations.Clear();

            foreach (var guid in locationGUIDs)
            {
                settings.locations.Add(AssetUtilities.LoadAsset<Location>(guid));
            }
        }
